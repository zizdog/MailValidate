...
/**
 * 邮箱验证用户后自动变为贡献者，配合Typecho多用户实现会员等级权限
 * 
 * @package MailValidate
 * @author zizdog
 * @version 0.0.1
 * @link https://blog.zizdog.com
 */
...
class MailValidate_Plugin implements Typecho_Plugin_Interface{
    ...
    public static function render(){
        ...
        if($row['validate_state']==="0"){
            echo '<span class="message" target="_self" style="background:#ff6a00;color:white;cursor:pointer;" onclick="window.location=\'/MailValidate/send\'">验证邮件提升权限</span>';
        }else if($row['validate_state']==="1"){
            echo '<span class="message" target="_self" style="background:#ff6a00;color:white;cursor:pointer;" onclick="window.location=\'/MailValidate/send\'">点此重发验证邮件</span>';
        }
    }
}
